...
export class TaskEngine {
  ...
  resumeTask(taskId: string): void {
    ...
    if (task.status === "COMPLETED" || task.status === "FAILED" || task.status === "CANCELLED") {
      throw new Error("Cannot resume a terminal task");
    }
    if (task.status !== "PAUSED" && task.status !== "WAITING_APPROVAL") {
  ...
  rejectTask(taskId: string, reason: string): void {
    ...
    if (!task) throw new Error("Task not found");
    if (task.status === "COMPLETED" || task.status === "FAILED" || task.status === "CANCELLED") {
      throw new Error("Cannot reject a terminal task");
    }
    if (task.status !== "PAUSED" && task.status !== "WAITING_APPROVAL") {
      throw new Error("Can only reject a task that is PAUSED or WAITING_APPROVAL");
    }
    if (task) {
      task.status = "FAILED";
    ...
    return new Promise((resolve) => {
      ...
      const runLoop = async () => {
        try {
          ...
          let wasPaused = false;
          while (execution.iterations < execution.maxIterations && !isTimeout) {

            // Re-fetch authoritative state from store every loop
            const storeTask = this.taskStore.getTask(task.id);
            if (!storeTask) break;
            task.status = storeTask.status;
            ...
            if (task.status === "PAUSED" || task.status === "WAITING_APPROVAL") {
              if (!wasPaused) {
                ...
                if (execution.state !== "WAITING_APPROVAL") {
                  this.recordStep(execution, "WAITING_APPROVAL", "Paused waiting for human intervention");
                }
                wasPaused = true;

                // Break out of the tight loop to prevent CPU lock.
                // The external controller must call resumeTask() to spawn a new runExecutionLoop.
                this.activeLoops.delete(task.id);
                return resolve(task);
              }
            }
